...
function youtube(){
	...
	local _dislikes=$(grep 'like-button-renderer-dislike-button'  "$_video" |sed -n 1p | sed 's/<[^>]*>//g;s/ //g')
	...
	local _subscribers=$(sed -n '/subscriber-count/{p;q;}' "$_channel" | sed 's/.*subscriber-count//g' | sed 's/<[^>]*>//g;s/.*>//g')
